...
from (
    select
        ...
        sum(amount) over (order by visited_on rows 6 preceding) as sum_amount -- 累计计算前6行，边界是当前行减去expr的值 最多计算6行
    from (
        select
            visited_on,
...
where
    datediff(visited_on, (select min(visited_on) from Customer)) >= 6 -- 找到最近的日期
```

> 核心代码块为`sum(amount) over (order by visited_on rows 6 preceding)`,其中的`rows 6 preceding`代表的是累计求和的是前6行
